 ...
 if [ $(mysql --login-path=local -N -s -e "select count(*) from information_schema.tables where \
    ...
    mysql --login-path=local -e "CREATE TABLE uf_shipments (id int NOT NULL AUTO_INCREMENT, userid int, trackingNumber varchar(50),shipDate int, origin varchar(1000),destination varchar(1000), currentLocation varchar(1000), eta int, status varchar(100), history TEXT(200000), PRIMARY KEY (id));"
fi
